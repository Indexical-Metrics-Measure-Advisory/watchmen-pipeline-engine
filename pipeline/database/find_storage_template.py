...
def find_storage_template():
    if settings.STORAGE_ENGINE == MONGO:
        from pipeline.database.table.base_table_definition import TableDefinition
        return StorageTemplate(TableDefinition())
    elif settings.STORAGE_ENGINE == MYSQL:
        from pipeline.database.table.mysql_table_definition import MysqlTableDefinition
        return StorageTemplate(MysqlTableDefinition())
    elif settings.STORAGE_ENGINE == ORACLE:
        from pipeline.database.table import oracle_table_definition
