...
from pipeline.monitor.model.pipeline_monitor import PipelineRunStatus, StageRunStatus
from pipeline.monitor.service import pipeline_monitor_service
...
from pipeline.core.parameter.parse_parameter import parse_parameter_joint
from pipeline.core.worker.stage_worker import run_stage
from pipeline.utils.constants import PIPELINE_UID, FINISHED, ERROR
from pipeline.topic.storage.topic_schema_storage import get_topic_by_id, get_topic_by_name
        ...
        if TriggerType.update.value in item:
            for update_data in item[TriggerType.update.value]:
                ...
                for key, data in merge_data.items():
                    pipeline.index.trigger_pipeline(topic_name, data, TriggerType.update, current_user,
                                                             trace_id)
        if TriggerType.insert.value in item:
            for insert_data in item[TriggerType.insert.value]:
                pipeline.index.trigger_pipeline(topic_name, insert_data, TriggerType.insert, current_user,
                                                         trace_id)
